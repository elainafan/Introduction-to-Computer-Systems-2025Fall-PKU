...
因此，在此采用同样的思想，即对``赋值的语句``跟``判断的语句``都进行编程，使其跳过当前语句，此处考虑只跳过单语句，对寄存器以及堆的影响降到最小。
